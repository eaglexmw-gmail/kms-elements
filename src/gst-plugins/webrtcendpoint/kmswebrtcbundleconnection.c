    ...
    GstBin * bin, gboolean active)
{
  ...
  gst_bin_add (bin, g_object_ref (&(tr->src->parent.element)));
  gst_bin_add (bin, g_object_ref (&(tr->sink->parent.element)));
}
    ...
    guint16 max_port, gchar * pem_cerficate)
{
  ...
  obj =
      g_object_new (KMS_TYPE_WEBRTC_BUNDLE_CONNECTION, "max-port", max_port,
...
static void
kms_webrtc_bundle_connection_collect_latency_stats (KmsIRtpConnection * obj,
    gboolean enable)
{
    ...
    iface)
{
  ...
  iface->collect_latency_stats =
      kms_webrtc_bundle_connection_collect_latency_stats;
}
